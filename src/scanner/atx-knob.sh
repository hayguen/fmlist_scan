...
while [ $R -ne 0 ]; do
  ...
  echo "waiting for termination of scanLoop .." >>${FMLIST_SCAN_RESULT_DIR}/fmlist_scanner/atx-knob.log
  sleep 1
  screen -ls
  R="$( screen -ls |grep -c scanLoopBg )"
  echo "scanLoop running: $R"
...
echo "ploadScanResults.sh .." >>${FMLIST_SCAN_RESULT_DIR}/fmlist_scanner/atx-knob.log
