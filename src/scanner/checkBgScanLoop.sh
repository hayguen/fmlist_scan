...
if [ $D -ge ${FMLIST_SCAN_DEAD_TIME} ]; then
  ...
  if [ ${FMLIST_SCAN_DEAD_REBOOT} -ne 0 ]; then
    ...
    if [ -z "$SSESSION" ]; then
      ...
      echo "${DTF}: checkBgScanLoop.sh: scanLoopBg screen session is not running! saving results, then restarting scanLoop .." >>${FMLIST_SCAN_RESULT_DIR}/checkBgScanLoop.log
      saveScanResults.sh savelog
      ...
      echo "${DTF}: checkBgScanLoop.sh: scanLoopBg screen session is hanging! killing session, saving results, then restarting scanLoop .." >>${FMLIST_SCAN_RESULT_DIR}/checkBgScanLoop.log
      stopBgScanLoop.sh
      resetScanDevice.sh all
      pkill scanLoop.sh
      pkill scanFM.sh
