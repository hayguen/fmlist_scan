...
function usage() {
  echo "usage: $0 <#minutes> <filenameId> <channel> [<additional options to ${DAB_RTLSDR_BIN##*/}>]"
  echo " additional options - as in ${DAB_RTLSDR_BIN##*/}, e.g. -P or -S:"
  "${DAB_RTLSDR_BIN}" -h
}
...
echo "starting   ${DAB_RTLSDR_BIN##*/} ${DABLISTENOPT} -n ${durationSeconds} -w ${FMLIST_SCAN_RAM_DIR}/${FN} -C $@"
...
timeout -v -s SIGTERM -k ${durationKill} ${durationClose} \
  "${DAB_RTLSDR_BIN}" ${DABLISTENOPT} -n ${durationSeconds} -w "${FN}" -C "$@" 2>&1 | tee ${FL}
